...
public class ItemBox : MonoBehaviour
{
    ...
    void DropItems()
    {
        if (ItemList == null || ItemList.Length == 0 || DropCount <= 0)
            return;

        for (int i = 0; i < DropCount; i++)
        {
            // ItemList에서 랜덤으로 아이템 선택
            GameObject selectedItem = ItemList[Random.Range(0, ItemList.Length)];
